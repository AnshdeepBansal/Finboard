/**
 * StatusBanner - Display API test status with appropriate styling
 */
export default function StatusBanner({ status, message, errorType }) {
  if (!status) return null;

  const isSuccess = status === 'success';
  const colorClasses = isSuccess
    ? 'bg-green-900/30 text-green-200 border-green-700/50'
    : errorType === 'rate_limit'
      ? 'bg-yellow-900/30 text-yellow-200 border-yellow-700/50'
      : errorType === 'auth_error' || errorType === 'permission_error'
        ? 'bg-orange-900/30 text-orange-200 border-orange-700/50'
        : 'bg-red-900/30 text-red-200 border-red-700/50';

  return (
    <div className={`mt-2 rounded-lg p-3 border ${colorClasses}`}>
      <div className="flex items-start gap-2">
        {isSuccess ? (
          <svg
            className="h-5 w-5 shrink mt-0.5"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
              clipRule="evenodd"
            />
          </svg>
        ) : errorType === 'rate_limit' ? (
          <svg
            className="h-5 w-5 shrink mt-0.5"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
              fillRule="evenodd"
              d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
              clipRule="evenodd"
            />
          </svg>
        ) : (
          <svg
            className="h-5 w-5 shrink mt-0.5"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
              clipRule="evenodd"
            />
          </svg>
        )}
        <span className="flex-1">{message}</span>
      </div>
    </div>
  );
}
